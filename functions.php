...
// Add query vars for products
function arontara_query_vars($vars) {
    $vars[] = 'product_category';
    $vars[] = 'product_name';
    return $vars;
}
add_filter('query_vars', 'arontara_query_vars');

// Template redirect for products
function arontara_template_redirect() {
    if (get_query_var('product_category') && get_query_var('product_name')) {
        include(get_template_directory() . '/single-product.php');
        exit;
    }
}
